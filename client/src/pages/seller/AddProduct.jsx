...
const AddProduct = () => {
    ...
    const onSubmitHandler= async(event)=>{
        try {
           ...
           console.log("Response from /api/product/add:", data);
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img src={files[index]? URL.createObjectURL(files[index]):assets.upload_area} className='max-w-24 cursor-pointer' alt="" />
                            </label>
                        ))}
                    </div>
                </div>
